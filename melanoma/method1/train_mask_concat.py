...
from melanoma.method1.data_mask_concat import load_rows, make_loaders, stratified_split
...
def main():
    ...
    model = build_classifier_mask_concat(args.backbone, pretrained=not args.no_pretrained)
    model.to(device)
    ...
    ckpt = args.checkpoint_dir / (args.backbone + "_mask_concat_last.pt")
    torch.save(
        {
            "model": model.state_dict(),
            "backbone": args.backbone,
            "with_mask_concat": True,
        },
        ckpt,
    )
    ...
    eval_data = LesionImageMaskConcatDataset(
        rows,
        args.image_dir,
        args.mask_dir,
        train=False,
    ...
    merge_train_report("method1_image_plus_mask", report)
