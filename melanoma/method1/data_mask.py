...
class LesionImageMaskDataset(torch.utils.data.Dataset):
...
def make_loaders(image_dir, mask_dir, label_csv, val_ratio, batch_size, num_workers, seed, use_weighted_sampler=True):
    ...
    if len(rows) == 0:
        raise ValueError("No images with masks found. Check paths.")

    train_idx, val_idx = stratified_split(rows, val_ratio, seed)

    train_data = LesionImageMaskDataset(rows, image_dir, mask_dir, train=True)
    val_data = LesionImageMaskDataset(rows, image_dir, mask_dir, train=False)
